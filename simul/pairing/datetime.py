...
df["NewCol1"], df["NewCol2"] = zip(*df.apply(lambda x: calculate(x['a'], x['b']), axis=1))
...
def calculateFltTime(x,y,z):
    ...
    dt1= datetime.datetime(flt_dt.year,flt_dt.month, flt_dt.day, int(y[0:2])-1, int(y[2:4]), 0, 0)
    dt2= datetime.datetime(flt_dt.year,flt_dt.month, flt_dt.day, int(z[0:2])-1, int(z[2:4]), 0, 0)
    ...
    if dt1 > dt2  :
        ...
        dt2= datetime.datetime(flt_dt.year,month, day, int(z[0:2])-1, int(z[2:4]), 0, 0)    
